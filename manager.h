...
public:
    ...
    void Play_Playlist() {
        Display_Manager();
        string playlistName;
        Node_manager* currentReader = nullptr;//pointer to current playist node

        //loop until valid playlist name is provided
        while (true) {
            cout << "Enter the playlist name: ";
            cin >> playlistName;
        ...
        lastPlaylist = temp1;
        //loop for playing surahs
        while (true) {
            //if the playlist has no surahs skip to the next playlist
            ...
            Node_playlist* currentSurah = currentReader->headplaylist;//start with first surah
            ...
            char c;
            bool playlistEnded = false; //flag to see if the playlist ended

            //loop for navegation
            while (true) {
                c = _getch();

                if (c == 75 && currentSurah->prev != nullptr) {//left arrow
                    engine->stopAllSounds();
            ...
            //handle reaching to the end of the playlist
            if (playlistEnded) {
                char userChoice;
